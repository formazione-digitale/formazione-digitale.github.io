...
# Incolla i sorgenti richiesti dopo il prompt, poi specifica il task.
...
- Hover: gradiente più scuro/profondo (#183D5F → #1F5580)
...
3. Aggiungi la regola hover: stesso selettore, principio
   "intensifica, non cambia"
4. Titolo e footer seguono il contrasto del tipo scelto
5. NON usare .card:not(.coming):hover per la logica del titolo —
...
- ❌ Usare `.card:not(.coming):hover .card-title` per cambiare
  il colore del titolo — si applica anche alla featured e
  schiarisce il testo bianco su sfondo scuro
- ❌ `transform: scale(1.22) rotate(-4deg)` sull'icona — esce
  dal bordo della card (overflow visivo)
- ❌ `overflow: hidden` sul .card-cta — blocca la freccia
  che non riesce a scorrere
- ❌ Nomi file/cartelle con spazi — rompono i link su
  GitHub Pages
- ❌ Usare la stessa regola hover per card-featured e
  card-active — i colori di sfondo e testo sono opposti
  e si sovrascrivono male

---

## SORGENTI ATTUALI

[INCOLLA QUI IL CONTENUTO DI index.html]

[INCOLLA QUI IL CONTENUTO DI EVENTUALI ALTRI FILE RILEVANTI]
...
Esempi:
- "Aggiungi una nuova scheda nella sezione Guide per questa risorsa: [titolo, descrizione, cartella, file]"
- "Crea una nuova guida HTML su [argomento] con la stessa grafica della Guida_Prompting.html"
- "Aggiungi una pillola su [argomento] nella sezione Pillole"
- "Modifica la card di [risorsa esistente] cambiando [cosa]"
- "Crea una nuova sezione nell'index per [tipo di contenuto]"
